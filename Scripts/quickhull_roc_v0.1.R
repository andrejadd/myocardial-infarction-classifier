...
quickhull_roc <- function(x,y) {
        ...
        for(mj in 1:(length(hull_x_set) - 1)) {
            ...
            for ( i in 1:length(x)) {
                ...
                if (DEBUG_OUTPUT) {
                    ...
                    if(max_i > 0) {
                        plot(x[max_i], y[max_i], pch=5, lwd=2, col="red"  , xlim=c(1,0), ylim=c(0,1))
                        par(new=T)
                    }
                    
                    ## plot current point
                    plot(x[i]    , y[i]    , pch=2, lwd=5,col="green", xlim=c(1,0), ylim=c(0,1))
                    
                    ## plot the hull segment, i.e.e the divide line, used to determine distant point
                    par(new=T)
                    plot(hull_x, hull_y, type="l", xlim=c(1,0), ylim=c(0,1))
                }
    ...
    return(list(x = hull_x_set, y = hull_y_set))
}
